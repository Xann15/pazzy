...
$seenClass = (!!$seen ? 'text-seen' : ''); // Tambahkan kelas jika dibaca

$timeAndSeen = "<span data-time='$created_at' class='message-time'>
        ".($isSender ? "<span class='fas fa-$seenIcon $seenClass' seen></span>" : '' )." <span class='time'>$timeAgo</span>
    </span>";

// Format pesan untuk mendukung tag <censored>
if ($message) {
    $message = str_replace('<censored>', '<span class="censored">', $message);
    $message = str_replace('</censored>', '</span>', $message);
}
?>
        ...
        @if (@$attachment->type != 'image' || $message)
            <div class="message">
                {!! ($message == null && $attachment != null && @$attachment->type != 'file') 
                    ? $attachment->title 
                    : nl2br($message) !!}
                <span style="float:right; bottom:">{!! $timeAndSeen !!}</span>
