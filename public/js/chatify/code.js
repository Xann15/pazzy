...
const messagesContainer = $(".messenger-messagingView .m-body"),
 ...
 msgTyping = $('.messenger-list-item tr td');
...
function cssMediaQueries() {
 ...
 if (window.matchMedia("(max-width: 980px)").matches) {
   ...
   $("body").find(".stories-list-item").find("div").attr("data-action", "1");
   ...
   $("body").find(".stories-list-item").find("div").attr("data-action", "0");
 }
}
...
clientListenChannel.bind("client-typing", function (data) {
 // Update daftar kontak untuk semua pengguna, bukan hanya yang sedang di-chat
 let contactItem = $(".messenger-list-item[data-contact='" + data.from_id + "']");

 if (data.typing == true) {
   // Tampilkan indikator "Typing..." di daftar kontak
   contactItem.find(".log-typing").show();
   contactItem.find(".last-message").hide(); // Sembunyikan last message sementara
 } else {
   // Sembunyikan indikator "Typing..." di daftar kontak
   contactItem.find(".log-typing").hide();
   contactItem.find(".last-message").show(); // Tampilkan kembali last message
 }

 // Jika sedang membuka chat dengan user yang mengetik, update chat window juga
 if (data.from_id == getMessengerId() && data.to_id == auth_id) {
   if (data.typing == true) {
...
clientListenChannel.bind("client-seen", function (data) {
 if (data.from_id == getMessengerId() && data.to_id == auth_id) {
   if (data.seen == true) {
     $(".message-time")
       .find(".fa-check")
       .before('<span class="fas fa-check-double seen text-seen"></span> ');
...
function getstoriesList() {
 $(".messenger-stories").html(avatarLoading(4));
 $.ajax({
   url: url + "/stories",
   method: "POST",
   data: { _token: csrfToken },
   ...
   success: (data) => {
     if (data.count > 0) {
       ...
       $(".messenger-stories").html(data.stories);
     } else {
...
// Pastikan AudioContext bisa digunakan
async function ensureAudioContext() {
 if (!window.chatify.audioContext) {
   window.chatify.audioContext = new (window.AudioContext || window.webkitAudioContext)();
 }

 if (window.chatify.audioContext.state === "suspended") {
   await window.chatify.audioContext.resume();
 }
}

// Fungsi untuk memainkan suara notifikasi
async function playNotificationSound(soundName, condition = false) {
 if ((document.hidden || condition) && chatify.sounds.enabled) {
   try {
     await ensureAudioContext();

     // Path file suara
     const soundUrl = `${window.location.origin}/sounds/chatify/${soundName}.mp3`;
     console.log("Trying to load sound from:", soundUrl);
     ...
     const audioBuffer = await window.chatify.audioContext.decodeAudioData(arrayBuffer);

     // Buat sumber suara dan hubungkan ke output
     const source = window.chatify.audioContext.createBufferSource();
     source.buffer = audioBuffer;
     source.connect(window.chatify.audioContext.destination);
     source.start(0);
...
// Event listener untuk mengaktifkan audio saat user berinteraksi
document.addEventListener("click", async () => {
 await ensureAudioContext();
 console.log("Audio context enabled!");
});

// Tambahkan tombol tes ke halaman (untuk debugging)
// const testButton = document.createElement("button");
// testButton.innerText = "Tes Suara";
// testButton.onclick = () => playNotificationSound("new-message-sound", true);
